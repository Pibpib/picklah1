...
export default function SpinScreen() {
  ...
  // 2) Load filters based on plan
  useEffect(() => {
    const loadFilters = async () => {
      const catData = await fetchCategories();
      const moodData = await fetchMoods();

      const cat = userPlan === "free" ? catData.filter(c => c.accessLevel === "free") : catData;
      const md  = userPlan === "free" ? moodData.filter(m => m.accessLevel === "free") : moodData;

      setCategories(cat);
      setMoods(md);
      setSelectedCategories(cat.map(c => c.id));
      setSelectedMoods(md.map(m => m.id));
    };
    loadFilters();
  }, [userPlan]);
